...
A side effect of choosing components that are resilient to changes in survey design is that if a circumstance arises that looks to break a variable name, it could be that you need a new question instead.
...
I prefer underscores, I think they're the easiest to read. Buy you also need to consider your end-user's software. Programs like Stata don't support dashes or dots in variable names, so you're kinda forced into using underscores anyway. Fine by me!
...
-   Do you use a different convention for administrative variables?
    -   Participant ID doesn't change over waves, so perhaps `pid` sufficient over `admin_pid`
